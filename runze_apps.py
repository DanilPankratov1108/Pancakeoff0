...
class App(tk.Tk):
    def __init__(self):
        ...
        self.running = True
        # Window settings
        self.title("MultiFlow GUI")
        self.geometry("800x480+0+0")
        #self.update_idletasks()
        #self.after(500, lambda: self.geometry("800x480+0+0"))
        ...
        # UI frames
        self.top_frame = tk.Frame(self, bg=self.bg_color, height=50, relief ="groove", bd=3)
    ...
    def show_recipe_stages(self):
        ...
        for idx, stage in enumerate(self.recipe_stages):
            ...
            f = tk.Frame(col_frames[col], bg="white", relief="solid", bd=2, width=100, height=30)
    ...
    def open_block_editor(self, stage_index, block_index):
        ...
        # syringe at top
        tk.Label(container, text="Объём шприца (мкл):", bg=self.bg_color, font=(self.font_family, 10)).pack(pady=4)
        syringe_var = tk.IntVar(value=data.get("syringe", 125))
        syringe_box = ttk.Combobox(container, textvariable=syringe_var, values=[125, 500], font=(self.font_family, 10), width=18)
        ...
        for key, label_text in labels:
            tk.Label(container, text=label_text, bg=self.bg_color, font=(self.font_family, 10)).pack(pady=3)
            if key == "direction":
                entry = ttk.Combobox(container, textvariable=direction_var, values=["Вперёд", "Назад"], font=(self.font_family, 10), width=18)
                entry.set(data.get("direction", "Вперёд"))
            else:
                entry = tk.Entry(container, font=(self.font_family, 10), width=20)
    ...
    def _execute_stage_once(self, stage_idx):
        ...
        for b_idx, block in enumerate(stage.get("blocks", [])):
            try:
                ...
                pump.set_volume(int(block.get("syringe")))
